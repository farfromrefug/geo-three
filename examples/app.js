...
function setNormalsInDebug(value) 
{
	...
	if (map) 
	{
		applyOnNodes((node) => 
		{
			node.material.userData.computeNormals.value = normalsInDebug;
...
function toggleDrawElevations(value) 
{
...
const dnormalsInDebugCheckBox = document.getElementById('normalsInDebug');
dnormalsInDebugCheckBox.onchange = (event) => {return setDebugMode(event.target.checked);};
dnormalsInDebugCheckBox.value = normalsInDebug;
...
const drawElevationsCheckbox = document.getElementById('drawElevations');
drawElevationsCheckbox.onchange = (event) => {return setDrawElevations(event.target.checked);};
drawElevationsCheckbox.value = drawElevations;
...
class MaterialHeightShader extends Geo.MapHeightNode 
{
	...
	{
		let size = MaterialHeightShader.getGeometrySize(level);
	...
	static getEdgeMorph(location) 
	{
		switch (location) 
		{
		case Geo.MapNode.TOP_LEFT:
			return Edge.LEFT | Edge.TOP;
		case Geo.MapNode.TOP_RIGHT:
			return Edge.RIGHT | Edge.TOP;
		case Geo.MapNode.BOTTOM_LEFT:
			return Edge.LEFT | Edge.BOTTOM;
		case Geo.MapNode.BOTTOM_RIGHT:
			return Edge.RIGHT | Edge.BOTTOM;
		}
	}

	static prepareMaterial(material, level, location) 
	{
		material.userData = {
			uEdgeMorph: {type: "i", value: MaterialHeightShader.getEdgeMorph(location)},
			uGrid: {type: "f", value: MaterialHeightShader.getGeometrySize(level)},
		...
		material.onBeforeCompile = (shader) => 
		{
			...
			shader.vertexShader = shader.vertexShader.replace(
				...
				// vec3 _transformed = position;
				// if( vMorphFactor > 0.0 ) {
				//   // Get position that we would have if we were on higher level grid
				//   float grid = 2.0 * uGrid;
				//   vec3 position2 = floor(_transformed / grid) * grid;
			  
				//   // Linearly interpolate the two, depending on morph factor
				//   _transformed = mix(_transformed, position2, vMorphFactor);
				// }
