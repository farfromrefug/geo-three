...
// @ts-ignore
window.THREE = THREE;
import * as Geo from '../build/geo-three';
...
class MaterialHeightShader extends Geo.MapHeightNode 
{
	...
	public static BASE_GEOMETRY = Geo.MapPlaneNode.GEOMETRY;

	public static BASE_SCALE: THREE.Vector3 = new THREE.Vector3(Geo.UnitsUtils.EARTH_PERIMETER, 1, Geo.UnitsUtils.EARTH_PERIMETER);
...
function createMap() 
{
	...
	provider.minZoom = 5;
	provider.maxZoom = 11;
	map = new Geo.MapView(new MaterialHeightShader(null, map, Geo.MapNode.ROOT, 0, 0, 0), provider, new LocalHeightProvider(), render);
	map.lod = lod;
